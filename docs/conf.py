...
project = 'reynard_the_robot'
copyright = '2024, Wason Technology LLC'
author = 'John Wason'
...
with readme_target.open("w") as outf:
    ...
    for line in readme_path.read_text().split("\n"):
        if line.startswith("# "):
            ...
            continue
        line = line.replace("docs/figures", "_static/figures/")
        line = line.replace("docs/", "")
        lines.append(line)
    outf.write("\n".join(lines))

# create static and figures directories
...
# copy figures
for fig in pathlib.Path(__file__).parent.resolve().glob("figures/*"):
    target = figures_dir / fig.name
